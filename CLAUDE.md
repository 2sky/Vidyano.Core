...
| `SAVE EXPECTING ERROR` / `ACTION <action> EXPECTING ERROR` / `OPEN PersistentObject\|Query\|MenuItem … EXPECTING ERROR` | Trailing suffix on the fallible verbs that asserts the **negative path**: the verb passes iff it fails as expected, and **fails if it unexpectedly succeeds** (`assert-expected-error`). For `SAVE`/`ACTION` the expected failure is a server error notification (`assert-notification-error`); only that notification is absorbed — a client-side guard (e.g. `guard-edit-mode-required` from SAVE before EDIT, or `guard-action-not-available`) or transport fault still fails. The notification stays on the current PO (or, for a query action, on the current Query), so a following `EXPECT Notification …` / `EXPECT Notification.Type = "Error"` pins the message. Composes with every ACTION form (`= option`, `(params)`, `Detail "<name>"`) and with `SAVE @initial`. All three **`OPEN`** forms take the suffix to assert the open is **refused**, each accepting a per-form error-kind set (the shared inversion is `WrapExpectingError(stmt, res, params kinds…)`; `params`-empty defaults to SAVE/ACTION's `AssertNotificationError`): `OPEN PersistentObject` → `{server-error}` (`DoOpenPo`); `OPEN Query` → `{resolve-query, server-error}` (`DoOpenQuery`); `OPEN MenuItem` → `{resolve-menu-item, resolve-query, server-error}` (`DoOpenMenu`, since a resolved leaf delegates to the PO/Query load). A client-side authoring guard (e.g. OPEN before SIGN-IN, `state-not-signed-in`) is **never** in the set, so it still fails. Two OPEN-only caveats: Core throws away the error PO/query so **no frame is pushed and `EXPECT Notification` cannot follow** (message only in the run diagnostic), and a refused open is **indistinguishable from a transport fault** (Core's `PostAsync` collapses every open failure into one error channel). |
